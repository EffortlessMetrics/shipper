...
| Idempotent workspace publish | advisory | `docs/specs/SHIPPER-SPEC-0007-idempotent-workspace-publish.md`; `docs/how-to/publish-missing-workspace-crates.md`; promotion blocked on focused JSON summary + idempotent publish regression suite | cli/integrations |
...
| Receipt-driven remediation | planned/advisory | `docs/specs/SHIPPER-SPEC-0008-receipt-driven-remediation.md`; `plans/0.4.0/receipt-driven-remediation.md`; bounded primitives above are mapped to proof, but full remediation remains planned until targeted `PackageYanked` event proof, `.shipper/remediation-plan.json` artifact contracts, and guarded execution proof exist | engine/cli |
