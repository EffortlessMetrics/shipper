...
| Idempotent workspace publish | stable | `cargo test -p shipper-cli --test bdd_publish --locked`; `cargo test -p shipper-core publish --lib --locked`; `shipper publish --format json` emits `published`/`skipped_existing`/`failed` summary fields and safety/artifact signals; `docs/how-to/publish-missing-workspace-crates.md`; `docs/specs/SHIPPER-SPEC-0007-idempotent-workspace-publish.md` | cli/integrations |
