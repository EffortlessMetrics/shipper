//! State file encryption module for Shipper.
//!
//! This module provides transparent encryption and decryption of state files using
//! AES-256-GCM with PBKDF2 key derivation from user passphrases.
//!
//! ## Usage
//!
//! The encryption is disabled by default. To enable it:
//! 1. Set `SHIPPER_ENCRYPT_KEY` environment variable, or
//! 2. Provide a passphrase via the config file or CLI
//!
//! ## Security
//!
...
pub fn read_decrypted(path: &std::path::Path, passphrase: &str) -> Result<String> {
...
pub fn write_encrypted(path: &std::path::Path, data: &[u8], passphrase: &str) -> Result<()> {
...
/// Transparent encryption wrapper for state file operations.
///
/// This provides a simple interface for encrypting/decrypting state files
...
impl StateEncryption {
    ...
    pub fn read_file(&self, path: &std::path::Path) -> Result<String> {
        if !self.is_enabled() {
            // Read as plain text
    ...
    pub fn write_file(&self, path: &std::path::Path, data: &[u8]) -> Result<()> {
        if !self.is_enabled() {
            // Write as plain text
