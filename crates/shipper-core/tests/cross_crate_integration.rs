...
fn sample_receipt(plan_id: &str) -> shipper_core::types::Receipt {
...
fn events_persisted_via_store_readable_via_state_module() {
    ...
    events.record(PublishEvent {
        ...
        event_type: EventType::PreflightComplete {
            finishability: shipper_core::types::Finishability::Proven,
        },
        package: "all".to_string(),
    });
    ...
    let events_file = shipper_core::state::events::events_path(&state_dir);
...
fn snapshot_plan(ws: &shipper_core::plan::PlannedWorkspace) -> PlanSnapshot {
    let packages = ws
        .plan
        .packages
        .iter()
