...
fn bdd_given_existing_pending_package_when_state_is_updated_and_persisted_then_state_is_written() {
    ...
    let mut st = ExecutionState {
        state_version: shipper_core::state::execution_state::CURRENT_STATE_VERSION.to_string(),
    ...
    let loaded = shipper_core::state::execution_state::load_state(temp_dir.path())
        .expect("load state")
        .expect("state exists");
    assert!(matches!(
