...
use shipper_core::state::events::EventLog;
use shipper_core::state::execution_state as state;
use shipper_core::store::{FileStore, StateStore};
...
fn sample_receipt(plan_id: &str, pkg_names: &[&str]) -> shipper_core::types::Receipt {
...
fn events_through_store_match_direct_event_log() {
    ...
    let events_file = shipper_core::state::events::events_path(td.path());
...
fn receipt_mixed_outcomes_through_file_store() {
    ...
    let receipt = shipper_core::types::Receipt {
        receipt_version: state::CURRENT_RECEIPT_VERSION.to_string(),
...
fn facade_snapshot_plan(ws: &shipper_core::plan::PlannedWorkspace) -> FacadePlanSnapshot {
    let packages = ws
        .plan
        .packages
        .iter()
...
fn receipt_all_fields_persisted_and_roundtrip() {
    ...
    let receipt = shipper_core::types::Receipt {
        ...
        event_log_path: std::path::PathBuf::from(".shipper/events.jsonl"),
        git_context: Some(shipper_core::types::GitContext {
