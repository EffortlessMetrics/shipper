...
fn receipt_version_too_old_rejected() {
    let result =
        shipper_core::state::execution_state::validate_receipt_version("shipper.receipt.v0");
...
fn receipt_version_invalid_format_rejected() {
    let result = shipper_core::state::execution_state::validate_receipt_version("garbage");
    assert!(result.is_err());
}
