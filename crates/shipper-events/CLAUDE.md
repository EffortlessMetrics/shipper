...
This file provides agent-specific guidance for working in crate shipper-events.

## Scope

- Crate: shipper-events
- Path: crates/shipper-events
...
```bash
cargo check -p shipper-events
cargo test -p shipper-events
cargo test -p shipper-events --all-features
cargo fmt -p shipper-events
cargo clippy -p shipper-events --all-targets --all-features -- -D warnings
```
