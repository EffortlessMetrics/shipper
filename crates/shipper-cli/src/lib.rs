//! Library façade of the `shipper` CLI.
//!
//! The sole public entry point is [`run`]: the installed binary is a
//! three-line shim that calls it. Keeping everything else `pub(crate)`
//! preserves freedom to refactor internals without breaking semver of
//! the `shipper` facade crate, which re-exports this [`run`].
...
/// Parse CLI arguments and dispatch into the appropriate command handler.
///
/// This is the single entry point for both the `shipper-cli` binary and
/// the `shipper` façade crate's binary. Keep the surface tight: no other
/// items in this crate are intended to be public.
pub fn run() -> Result<()> {
    let cli = Cli::parse();
